                ...
                <div className="card-body minHight">
...

                    {
                        url == _id ?
                            ...
                            :
                            <p className="card-text">{newDes ? <>{newDes} <button id={_id} onClick={handleModal} className='seemore' data-bs-toggle="modal" data-bs-target="#exampleModal">...see more</button></> : description}</p>
                    }
                </div>
                <div
                    className="d-flex justify-content-lg-between align-items-center justify-content-md-between justify-content-between  mx-lg-2 mx-md-2 mx-sm-2 mx-2">

                    <h2 className='price'>${price}</h2>
                    <h2 className={quantity === '0' ? 'price text-danger fs-5' : 'price fs-4'}>
                        <span>
                            <i className={quantity === '0' ? 'fas fa-sort-amount-up text-danger' : 'fas fa-sort-amount-up text-dark'}></i>
                        </span>
                        <span >{quantity === '0' ? 'Sold Out' : quantity}</span></h2>
                    {url === _id ?
                        <></>
                        :
